...
TEST_F(ParserTest, ListAsStruct) {
  Scanner scanner(".(1,[])");
  Parser parser(scanner);
  Term* termPtr = parser.createTerm();
  ASSERT_EQ("2", std::to_string(termPtr->arity()));
  ASSERT_EQ(".(1, [])", termPtr->symbol());
  ASSERT_EQ("1", termPtr->args(0)->symbol());
  ASSERT_EQ("[]", termPtr->args(1)->symbol());
}
...
TEST_F(ParserTest, ListAsStruct2) {
  ...
  Term* termPtr = parser.createTerm();
  ASSERT_EQ("2", std::to_string(termPtr->arity()));
  ASSERT_EQ(".(2, .(1, []))", termPtr->symbol());
  ASSERT_EQ("2", termPtr->args(0)->symbol());
  ASSERT_EQ(".(1, [])", termPtr->args(1)->symbol());
}
