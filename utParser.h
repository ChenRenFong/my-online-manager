...
TEST_F(ParserTest, ThreeTermsMatching) {
  ...
  EXPECT_EQ("Y", et->right->left->left->term->symbol());
  EXPECT_EQ("2", et->right->left->right->term->symbol());
  EXPECT_EQ("Z", et->right->right->left->term->symbol());
  EXPECT_EQ("3", et->right->right->right->term->symbol());
